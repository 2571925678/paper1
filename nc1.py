...
def nc(model, x, y_target, preprocess):
    ...
    for step in range(steps):
        ...
        if step % 10 == 0:
            print(
                f'Y_target: {y_target}, Step: {step}, cost: {cost:.4f}, CE_LOSS: {CE_LOSS:.4f}, REG_LOSS: {REG_LOSS:.4f}, ASR: {asr * 100:.2f}%')

        if asr >= asr_threshold and REG_LOSS < reg_best:
            px_best = px.detach().cpu()
            savefig = torch.cat([torch.repeat_interleave(mask_tanh, 3, dim=1), patn_tanh, patn_tanh * mask_tanh],
                                dim=0).detach().cpu()
            reg_best = REG_LOSS
...
def main(args):
    ...
    l1_norm_list = []
    results = []  # To save detailed results

    for y_target in inspect_classes:
        print(f'Currently inspecting class: {y_target}')
    ...
    # Outlier detection
    anomaly_index, flag_list = outlier_detection(inspect_classes, l1_norm_list)

    # Save results to nc_out/nc_out1.txt
    output_dir = "nc_out"
    os.makedirs(output_dir, exist_ok=True)  # Create directory if it doesn't exist
    output_filepath = os.path.join(output_dir, "nc_out7.txt")

    with open(output_filepath, "w") as f:
        f.write("Anomaly Detection Results\n")
        f.write("=========================\n\n")
        f.write(f"Anomaly Index: {anomaly_index:.4f}\n\n")
        # Add flagged label list
        f.write("\nFlagged Label List\n")
        f.write("=========================\n\n")
        if flag_list:
            for y_label, l_norm in flag_list:
                f.write(f"Class {y_label}: L1 Norm = {l_norm:.4f}\n")
        else:
            f.write("No flagged labels.\n")

        f.write("Inspect Classes and L1 Norms:\n")
        for i, (inspect_class, l1_norm) in enumerate(zip(inspect_classes, l1_norm_list)):
            f.write(f"Class {inspect_class}: L1 Norm = {l1_norm:.4f}\n")
        #
        # # Add flagged label list
        # f.write("\nFlagged Label List\n")
        # f.write("=========================\n\n")
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/clean_cifar10.pt', help='clean path1')
    # parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/lotus_best_cifar10_1029.pt',help='lotus_yuan1')
    # parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/11071816/lotus_best.pt',help='lotus_my1.82')
    # parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/11162216/lotus_best.pt',help='lotus_my2.29')
    # parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/11062025/lotus_best.pt',help='lotus_my2.8')
    # parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/11061929wshape/lotus_best.pt',help='lotus_my4')
    # parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/11191601/lotus_best.pt',help='lotus_my3.8')
    # parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/11062041/lotus_best.pt',help='lotus_my2.67')
    # parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/11191957/lotus_best.pt',help='lotus_my')
    parser.add_argument('--model_filepath', type=str, default='checkpoint/cifar10/11190942/lotus_best.pt',help='lotus_my2.67')
