    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 2 == 0:
            ...
            with torch.no_grad():
                ...
                total_bd = 0
                for (x_test, y_test,*_) in poison_test_loader:
                    x_test = x_test.cuda()
                    y_test = y_test.cuda()
    ...
    if args.network == 'wrn':
        ...
        if args.network == 'vgg11':
            ...
            net_pruned.layer4[1].shortcut = nn.Sequential(
                nn.Conv2d(pruning_mask.shape[0], pruning_mask.shape[0] - num_pruned, kernel_size=1, stride=1,
                          bias=False),
...
def anp(args, model, train_loader, test_loader, poison_test_loader, preprocess):
    ...
    for name, module in model.named_modules():
        if args.network == 'vgg11':
            target_layers = ['features.1', 'features.5', 'features.9', 'features.12', 'features.16', 'features.19',
                             'features.23', 'features.26']
...
def main(args, preeval=True):
    ...
    # Load attacked model
    model_filepath = f'checkpoint/cifar10/lotus_best_cifar10_1029.pt'
    # model_filepath = f'checkpoint/cifar10/clean_cifar10.pt'
    # model_filepath = f'checkpoint/cifar10/11062025/lotus_best.pt'
    ...
    poison_loader = DataLoader(dataset=poison_set, batch_size=args.batch_size)
    # test_set, _ = random_split(test_set, [1000, len(test_set) - 1000])
    ...
    acc, asr = test(model, test_loader, poison_loader, preprocess)
    print(
        f'{args.phase} :: Dataset: {args.dataset}, Network: {args.network}, Attack: {args.attack} --- ACC: {acc * 100:.2f}%, ASR: {asr * 100:.2f}%, Time: {total_time:.2f}s')
    return acc, asr
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attack', default='1', help='attack method')
